"""Rule that normalizes spacing, ellipsis, and punctuation in French text."""
...
class SpacingRule(Rule):
    """Apply French typographic spacing conventions."""

    def __init__(self) -> None:
        """Register the rule in the orchestrator."""
        super().__init__(name="spacing", config_attr="spacing")

    def detect(self, text: str) -> list[RuleResult]:
        """Return warnings for spacing inconsistencies in the provided text.

        Args:
            text: Text fragment to inspect.

        Returns:
            A list of rule results describing missing thin spaces, ASCII
            ellipsis, and other spacing violations.
        """
        out: list[RuleResult] = []
        ...
        for match in re.finditer(r"(?<!\u00A0|\u202F)([:;!?»])", text):
            ...
            out.append(
                (
                    match.start(),
                    match.end(),
                    f"Espace {exp} manquante avant «{char}»",
                    None,
                )
            )
    ...
    def fix(self, text: str) -> str:
        """Normalize spacing, apostrophes, and ellipsis characters in-place.

        Args:
            text: Text fragment to mutate.

        Returns:
            The corrected text with French spacing conventions applied.
        """
